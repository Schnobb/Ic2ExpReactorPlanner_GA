...
public class EvolutionEngine {
    ...
    public ArrayList<EvaluatedGenome> Run() {
        ...
        while (generation < this.config.evolution.maxGeneration) {
            ...
            for (int i = 0; i < this.config.evolution.alphaCount; i++) {
                EvaluatedGenome alpha = population.get(i);
            ...
            if (generation > 0 && generation % this.config.evolution.phaseLengthGeneration == 0)
                exploratoryPhase = !exploratoryPhase;

            int tournamentCount = this.config.evolution.populationSize - newPopulation.size();
            for (int i = 0; i < tournamentCount; i++) {
                EvaluatedGenome parentA;
                EvaluatedGenome parentB;
                ...
                tournamentSelection.sort(Comparator.comparingDouble(EvaluatedGenome::getFitness).reversed());
                parentA = tournamentSelection.get(0);
                ...
                tournamentSelection.sort(Comparator.comparingDouble(EvaluatedGenome::getFitness).reversed());
                parentB = tournamentSelection.get(0);

                // 50/50 chance to flip parentA and parentB
                if (this.random.nextDouble() < 0.5) {
                    EvaluatedGenome temp = parentA;
                    parentA = parentB;
                    parentB = temp;
                }
